  ...
  return (
    <div>
      <div className='flex justify-between p-6 text-white bg-slate-950'>
        ...
        {isProfileComplete ? (
          <div className='flex'>
            <h1 className='px-2 py-1 cursor-pointer' onClick={handleEditProfile}>
              {userDisplayName}!
            </h1>
            <PremiumButton expenses={expenses} />
            <button
              onClick={handleToggleTheme}
              className={`ml-6 p-2 rounded-lg ${isDarkTheme ? 'bg-gray-300 text-gray-800' : 'bg-gray-800 text-white'}`}
            >
              Theme
            </button>
            <div>
              <button className='ml-6 bg-blue-500 p-2 rounded-lg hover:bg-sky-800' 
